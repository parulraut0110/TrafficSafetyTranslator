...
# import uuid
# from fuzzywuzzy import process  # For better chatbot response matching
...
#     # Use fuzzy matching to improve response accuracy
#     best_match = process.extractOne(message, chatbot_responses.keys(), score_cutoff=70)
#     response = chatbot_responses.get(best_match[0], "I'm sorry, I don't understand that.") if best_match else "I'm sorry, I don't understand that."
    
#     # Translate the chatbot response to the selected language
#     try:
#         translated_response = translator.translate(response, dest=language).text
#     except Exception as e:
#         print(f"Error translating chatbot response: {e}")
#         translated_response = response  # Default to English if translation fails
...
#             file_path = save_file_with_unique_name(file)
...
#                 translation = "Error in translation process."

#         # Handle chatbot response
#         user_message = request.form.get('chat_message', '').lower()
#         if user_message:
#             chatbot_response = get_chatbot_response(user_message, language)
...
# Enhanced response for Generative AI
def get_gemini_response(question):
    ...
    try:
        ...
        print(f"Error in Generative AI response: {e}")
        return "I'm sorry, I couldn't process that. Please try again later."

# Handle file uploads for translation
...
=======
    ...
    if request.method == "POST":
        # Handle translation from text or file
        text = request.form.get('text', '')
        language = request.form.get('language', 'en')
        ...
        # Handle file upload translation
        file = request.files.get('file')
        if file and allowed_file(file.filename):
            filename = secure_filename(file.filename)
            file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
            file.save(file_path)
            translation = translate_file(file_path, language)

        # Handle Generative AI chatbot
        chat_message = request.form.get('chat_message', '')
        if chat_message:
            generative_response = get_gemini_response(chat_message)

    return render_template("index.html", translation=translation, generative_response=generative_response)
>>>>>>> fb362cc (Initial commit)
